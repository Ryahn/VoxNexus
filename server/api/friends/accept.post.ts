...
export default defineEventHandler(async (event) => {
  ...
  emitToUser(request.from.toString(), 'friend:request:accepted', { from: userPayload.id, username: userPayload.username });
  emitToUser(userPayload.id, 'friend:request:accepted', { from: request.from.toString() });
