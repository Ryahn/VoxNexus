...
export default defineEventHandler(async (event) => {
  ...
  if (!name) {
    return { status: 400, error: 'Channel name is required.' };
  }
  ...
  return { status: 201, channel: { id: channel._id, name: channel.name, type: channel.type, createdAt: channel.createdAt } };
}); 