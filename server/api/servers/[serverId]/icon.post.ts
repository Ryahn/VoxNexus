...
export default defineEventHandler(async (event) => {
  ...
  if (server.ownerId.toString() !== userPayload.id) {
    return { status: 403, error: 'Only the server owner can update the icon.' };
  }
  ...
  const iconPath = await attachmentService.uploadServerIcon(file.data, serverId, file.type || 'image/png');
  const publicUrl = `/${iconPath}`;

  server.iconUrl = publicUrl;
  await server.save();
